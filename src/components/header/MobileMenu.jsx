...
const MobileMenu = ({ isOpen, toggle }) => {
  ...
  return (
    ...
    <ul className={`lg:hidden | fixed text-2xl font-slim uppercase bg-black p-7 flex flex-col justify-center  h-screen w-full top-0 transition-all duration-700 ease-in-out ${isOpen ? 'left-0' : 'left-[-1000px]'} z-10`}>
      <li className='py-4'><Link to='/'>About</Link></li>
      <li className='py-4'><Link to='/careers'>Careers</Link></li>
      <li className='py-4'><Link to='/events'>Events</Link></li>
      <li className='py-4'><Link to='/products'>Products</Link></li>
      <li className='py-4'><Link to='/support'>Support</Link></li>
    </ul>
    </>
